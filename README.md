...
- **GPU processing:** All raster filters and adjustment variants prefer compute for substantial workloads, including full-resolution Apply, export/merge composition, resampling, masks, large paint/retouch strokes, and RAW development through 8/16-bit output. Small edits, codecs, text, and sequential fill algorithms stay on CPU; unavailable compute and resource failures fall back to CPU. See the [processing audit and benchmarks](docs/GPU_PROCESSING.md).
- **Filters:** Gaussian and Motion Blur with expanded bounds, Add Noise, Lens Correction, content-aware fill, and edge-color background removal. Motion Blur previews sample cached GPU textures directly; selections and clipping dependencies use the materialized filter worker. Filtering and expensive retouching run in cancellable workers.
...
The Motion Blur benchmarks measure live GPU preview updates and full-resolution Apply, including GPU readback, against CPU filtering at distances 15 and 200. Apply uses original layer pixels even when the preview texture is downsampled.

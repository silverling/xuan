...
- **Filters:** Gaussian and Motion Blur with expanded bounds, Add Noise, Lens Correction, content-aware fill, and edge-color background removal. Motion Blur previews use cached GPU textures, with a CPU fallback for selections and layers used as clipping sources. Apply filters the full-resolution layer on the GPU and reads the result back once; device limits or GPU failures fall back to CPU processing. Full-resolution filtering, healing, filling, and background removal run in cancellable workers.

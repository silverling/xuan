...
Portable release archives include a `source` directory containing Xuan's build sources and the exact Rawler sources used to build the binary. Debian and RPM packages include the same directory in `/usr/share/doc/xuan/source.tar.gz`; extract it into a writable directory first. To rebuild or relink with a modified Rawler, edit `source/vendor/rawler`, then run `cargo build --release` in `source`. A Rust toolchain, a C toolchain, the Linux libraries listed in the [development guide](docs/DEVELOPMENT.md#prerequisites), and access to the Cargo registry for other dependencies are required. The included manifest patches Rawler to the local sources; Cargo adjusts the lockfile for this path dependency on the first build.

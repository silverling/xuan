...
- **AppImage desktop libraries**: Linux AppImages also bundle X11/Wayland client libraries and their dependencies. Their copyright and license notices are included under `usr/share/doc/` and `usr/share/licenses/` inside the image; extract it with `--appimage-extract` to inspect them.

...
- HEIC/HEIF import uses the bundled `heic-rs` decoder on both platforms. Regression tests use small, licensed synthetic HEIC fixtures for color, tiled primary images, orientation, malformed input, size limits, and project persistence. Libheif's external 1280×854 example was also decoded; that image is not bundled.

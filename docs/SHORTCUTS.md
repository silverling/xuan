...
NEF/NRW files enter RAW Develop before becoming layers. In Develop, Ctrl+Z / Ctrl+Shift+Z undo and redo RAW settings; Escape exits the white-balance picker or mask drawing. Drag pans, wheel zooms, and the Fit / 100% buttons set the inspection scale. In Split view, drag moves the comparison divider and Alt-drag pans. Double-click a RAW layer (or its image with Move selected) to reopen Develop. Use Develop to commit, or Cancel to retain the previous layer state.

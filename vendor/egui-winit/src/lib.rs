        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed {
                if is_cut_command(self.egui_input.modifiers, active_key) {
                    ...
                    self.egui_input.events.push(paste_event(self.clipboard.get()));
                    return;
                }
            }
...
fn paste_event(contents: Option<String>) -> egui::Event {
    // Image editors must receive the paste command even without clipboard text.
    // egui text edits already ignore empty paste payloads.
    egui::Event::Paste(contents.unwrap_or_default().replace("\r\n", "\n"))
}

#[test]
fn clipboard_paste_preserves_commands_without_text() {
    for contents in [None, Some(String::new())] {
        assert_eq!(paste_event(contents), egui::Event::Paste(String::new()));
    }
    assert_eq!(
        paste_event(Some("line one\r\nline two".into())),
        egui::Event::Paste("line one\nline two".into())
    );
}
